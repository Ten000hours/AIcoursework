...
def environment():
    ...
    print(problem_id)
    # problem_id=0
    env = LochLomondEnv(problem_id=problem_id, is_stochastic=False, reward_hole=-0.01)
    epsilon = 0.9
...
def environment_eval(problem_id):
    ...
    # problem_id=0
    env = LochLomondEnv(problem_id=problem_id, is_stochastic=True, reward_hole=-0.01)
    epsilon = 0.9
    total_episodes = 50000
    max_steps = 100
...
def filegenerate(problem_id, Q):
    filename = "out_rl_stochastic" + str(problem_id)
    text = Q
    print(str(text))
